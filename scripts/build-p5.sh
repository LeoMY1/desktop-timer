...
if [ "$configuration" != "test" ] && [ "$configuration" != "release" ]; then
  printf 'Usage: bash scripts/build-p5.sh test|release\n' >&2
  exit 2
fi
build_root="$project_root/work/p5-build/$configuration"
bundle="$project_root/outputs/P5-v1/StudyTimer.app"
flags=(-D PRODUCTION)
if [ "$configuration" = "test" ]; then
  flags=(-D INTERNAL_TESTING)
  bundle="$project_root/work/p5-build/test/StudyTimer-Checks.app"
fi
...
cp "$project_root/resources/Info-P5.plist" "$bundle/Contents/Info.plist"
if [ "$configuration" = "test" ]; then
